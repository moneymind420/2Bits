...
Dependencies: Java 8 needs to be installed first. Oracle JVM gives
...
Using the 2Bitscoin package:

Unpack the 2Bitscoin zip package and open a shell in the resulting 2Bitscoin
...
message "2Bitscoin server 1.x.x started successfully" in the console log. If run in
...
where the 2Bitscoin UI should now be available.
...
to the 2Bitscoin installation directory, as the use of special characters may result
...
The 2Bitscoin software is a client-server application. It consists of a java server
...
requests which other 2Bitscoin client applications could use.
...
The default 2Bitscoin client does not store any wallet-type file on disk. Unlike
